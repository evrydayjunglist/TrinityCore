--
-- Archaeology (from the ground up) Phase 1 sub-slice 3a: dig-site -> research branch mapping.
-- Each dig site is themed to one race/branch; this is server-owned reference data not carried by
-- client DB2. Sourced from the Archy addon dig-site DB (blobID = ResearchSite.QuestPOIBlobID),
-- joined to ResearchBranch by fragment currency. Cataclysm slice = Eastern Kingdoms + Kalimdor.
-- findCount = 6 (retail 12.0.7.68453 survey sniff, Cata sites).
--
CREATE TABLE IF NOT EXISTS `archaeology_dig_site` (
  `researchSiteId` int unsigned NOT NULL DEFAULT '0' COMMENT 'ResearchSite.db2 ID',
  `researchBranchId` int unsigned NOT NULL DEFAULT '0' COMMENT 'ResearchBranch.db2 ID',
  `findCount` tinyint unsigned NOT NULL DEFAULT '6' COMMENT 'finds required to exhaust the site',
  PRIMARY KEY (`researchSiteId`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Archaeology dig site to research branch (fork reference data)';
DELETE FROM `archaeology_dig_site`;
INSERT INTO `archaeology_dig_site` (`researchSiteId`,`researchBranchId`,`findCount`) VALUES
(9,1,6),
(10,1,6),
(12,1,6),
(13,3,6),
(15,1,6),
(18,8,6),
(19,1,6),
(20,1,6),
(21,3,6),
(22,1,6),
(23,8,6),
(24,8,6),
(25,8,6),
(26,8,6),
(27,8,6),
(144,1,6),
(146,1,6),
(150,1,6),
(152,8,6),
(154,3,6),
(163,4,6),
(165,3,6),
(167,4,6),
(169,4,6),
(171,4,6),
(173,4,6),
(175,4,6),
(177,4,6),
(179,4,6),
(181,3,6),
(183,1,6),
(185,4,6),
(187,4,6),
(189,3,6),
(191,4,6),
(193,4,6),
(195,3,6),
(197,4,6),
(199,3,6),
(201,4,6),
(203,3,6),
(205,3,6),
(207,1,6),
(209,1,6),
(211,1,6),
(213,1,6),
(215,3,6),
(217,8,6),
(219,4,6),
(221,3,6),
(223,8,6),
(225,8,6),
(227,8,6),
(229,8,6),
(231,8,6),
(233,8,6),
(235,8,6),
(237,3,6),
(239,8,6),
(241,8,6),
(243,8,6),
(245,8,6),
(247,3,6),
(249,3,6),
(251,3,6),
(259,3,6),
(261,3,6),
(279,4,6),
(281,4,6),
(283,4,6),
(285,4,6),
(287,4,6),
(289,4,6),
(291,4,6),
(293,4,6),
(295,4,6),
(297,4,6),
(299,4,6),
(301,4,6),
(303,4,6),
(305,4,6),
(307,4,6),
(309,3,6),
(313,4,6),
(315,8,6),
(317,8,6),
(319,8,6),
(321,8,6),
(323,3,6),
(325,3,6),
(327,3,6),
(329,3,6),
(331,3,6),
(333,3,6),
(335,3,6),
(337,4,6),
(461,4,6),
(463,4,6),
(465,4,6),
(467,4,6),
(469,4,6),
(477,1,6),
(479,1,6),
(481,1,6),
(485,7,6),
(487,7,6),
(489,7,6),
(491,7,6),
(493,7,6),
(495,7,6),
(497,7,6),
(499,7,6),
(501,7,6),
(570,7,6),
(572,7,6),
(574,7,6),
(576,7,6),
(578,7,6),
(581,7,6),
(583,7,6),
(615,5,6),
(617,5,6);
