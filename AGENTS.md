...
- `docs/midnight-assessment/upstream-core-maintenance/upstream-core-maintenance-cypher-skill-handoff.md` — ad-hoc playtest fix: "Language: Cypher" auto-grants to every character and levels alongside them to L80 (**root cause found** 2026-07-01; **not yet fixed**, logged for follow-up)
- `docs/midnight-assessment/upstream-core-maintenance/upstream-core-maintenance-item-squish-bonus-types-handoff.md` — Midnight item-level-offset `ItemBonusType` 52/53 have no enum value or switch case in `Item.cpp` (**confirmed live gap** 2026-07-01 — 43+7 real rows in build `12.0.7.67808` data, wired into live crafting-quality bonus trees; not yet fixed); TC's core squish-era mechanism itself is already correctly implemented, confirmed no missing historical-squish logic
- `docs/midnight-assessment/playerbots/playerbots-bot-bnet-account-fk-handoff.md` — Playerbots bot sessions hardcode `battlenetAccountId=0` (**root cause found** 2026-07-01; **not yet fixed**), breaking every Bnet-account-scoped save (upstream `CollectionMgr`/`BattlePetMgr` transmog/toys/pets **and** this fork's own warbands `AccountCurrencyMgr`/`WarbandGroupMgr`) with a silent, repeating FK violation the moment a bot touches that data — confirmed live via `battlenet_item_appearances` (270 lines in `DBErrors.log`); warbands' own tables share the vulnerability but haven't been hit by a bot yet
