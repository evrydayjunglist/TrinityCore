...
#define BnetGameAccountInfo "a.id AS account_id, a.username, ab.bandate, ab.unbandate AS account_unbandate, ab.unbandate = ab.bandate AS is_banned, aa.SecurityLevel"
    ...
    PrepareStatement(LOGIN_SEL_BNET_ACCOUNT_CURRENCY, "SELECT Currency, Quantity, WeeklyQuantity, TrackedQuantity, IncreasedCapQuantity, EarnedQuantity, Flags FROM battlenet_account_currency WHERE battlenetAccountId = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(LOGIN_DEL_BNET_ACCOUNT_ACHIEVEMENT_BY_ACHIEVEMENT, "DELETE FROM battlenet_account_achievement WHERE battlenetAccountId = ? AND achievement = ?", CONNECTION_BOTH);
    ...
    PrepareStatement(LOGIN_DEL_BNET_ACCOUNT_ACHIEVEMENT_PROGRESS_BY_CRITERIA, "DELETE FROM battlenet_account_achievement_progress WHERE battlenetAccountId = ? AND criteria = ?", CONNECTION_BOTH);
}
